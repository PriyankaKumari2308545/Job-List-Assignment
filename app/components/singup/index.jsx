...
function Signup({ modal, setModal, setLogin }) {
  ...
  const icons = [<IoIosPerson />, <PiBagSimpleFill />];
    ...
    <Modal isOpen={modal} onClose={() => setModal(false)}>
      ...
      <div className="mt-10 mb-5 flex w-[100%] gap-4">
        {active.map((item, index) => (
          <Button
            buttonType={item.active == true ? "success" : "secondry"}
            onClick={() => handleActive(item.id)}
          >
            <div className="flex justify-center items-center py-2 gap-2 text-base">
              {icons[index]}
